...
public class Board extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private static final String PIECE_SOUND_FILE = "p.wav";
    private Clip pieceSoundClip;
    ...
    public Board(GameWindow g) {
        this.g = g;  // Assign the GameWindow reference

        // Load the sound file
        // Load the sound file using InputStream
        try {
            InputStream audioInputStream = getClass().getClassLoader().getResourceAsStream(PIECE_SOUND_FILE);
            if (audioInputStream != null) {
                AudioInputStream ais = AudioSystem.getAudioInputStream(new BufferedInputStream(audioInputStream));
                pieceSoundClip = AudioSystem.getClip();
                pieceSoundClip.open(ais);
            } else {
                System.err.println("Audio file not found: " + PIECE_SOUND_FILE);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        whiteTurn = true;  // Initialize the turn
    ...
    if(sfx == true) {
        if (pieceSoundClip != null) {
            // Rewind the sound clip to the beginning
            pieceSoundClip.setFramePosition(0);

            // Play the sound
            pieceSoundClip.start();
        }
    }
    }
